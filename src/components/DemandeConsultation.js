...
import { FaRegAddressCard, FaRegCalendarAlt, FaRegClock, FaCheck } from "react-icons/fa";
import { Modal, ModalBody, ModalFooter } from 'reactstrap';
import { NotificationContainer, NotificationManager } from 'react-notifications';

export default class DemandeConsultation extends Component {
    constructor(props) {
        super(props);
        this.state = {
            reservations: [],
    ...
    componentDidMount() {
        axios({
            method: 'POST',
            url: 'http://localhost/apiSante/demandeConsultation.php',
            headers: {
                ...
                {this.state.reservations.length > 0 ? (
                    this.state.reservations.map((el, index) => (
                        <div className="ligneReservation" key={index}>
                            ...
                            <button className="ValiderReservation" onClick={() => this.validerPatient(el.id_reservation, index)}><FaCheck /><span>Valider</span></button>
